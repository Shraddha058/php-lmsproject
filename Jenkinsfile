pipeline {
  ...
  environment {
    ...
    ECR_REPO       = "marketing-service"
    K8S_CLUSTER    = "marketing-eks"
    K8S_NAMESPACE  = "marketing"
    K8S_DEPLOYMENT = "marketing-service"
    CONTAINER_NAME = "app"
  ...
  stages {
    ...
    stage("Test") {
      steps {
        sh "docker run --rm ${IMAGE_NAME}:${IMAGE_TAG} php -m | grep -E 'pdo_mysql|mbstring|intl'"
      }
    }
